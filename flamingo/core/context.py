...
class Context(OverlayObject):
    ...
    def pre_render(self, content, template_context=None):
        if (
            not self.settings.PRE_RENDER_CONTENT
            or not content.get("is_template", True)
            or content["_content_body_rendered"]
        ):
    ...
    def render(self, content, template_name=""):
        ...
        self.plugins.run_hook("template_context_setup", content, template_name, template_context)
    ...
    def build(self, clean=True):
        ...
        if self.settings.CONTENT_PATHS:
            contents = self.contents.filter(
                Q(path__in=self.settings.CONTENT_PATHS) | Q(i18n_path__in=self.settings.CONTENT_PATHS),
            )
