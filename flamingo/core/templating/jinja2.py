    ...
    **kwargs,
    ...
    if i18n and ("flamingo.plugins.I18N" in context["context"].settings.PLUGINS):
        ...
        if content["lang"] != lang:
            content = context["context"].contents.get({i18n_content_key: content[i18n_content_key], "lang": lang})
...
class Jinja2(TemplatingEngine):
    def __init__(self, context):
        ...
        # setup env
        template_dirs = [os.path.join(i, "templates") for i in self.theme_paths] + [self.tempdir.name]
        ...
        # setup error env
        if self.context.settings.LIVE_SERVER_RUNNING and self.context.settings.JINJA2_TRACEBACKS:
            autoescape = not self.context.settings.JINJA2_TRACEBACKS_PYGMENTS

            self.error_env = Environment(
                loader=FileSystemLoader([os.path.join(THEME_ROOT, "templates/jinja2")]),
    ...
    def _render_exception(self, exception):
        ...
        for frame in traceback.extract_tb(exception.__traceback__)[::-1]:
            ...
            if filename in self.contents:
                ...
                if not content_path and self.contents[filename]["original_path"]:
        ...
        if isinstance(exception, TemplatingError):
            ...
            if exception.content_path and exception.content_lineno:
                ...
                stack = [(content_abspath, exception.content_lineno, "")]
    ...
    def _render(self, template_name, template_context, handle_exceptions=True):
        if not self.context.settings.LIVE_SERVER_RUNNING or not self.context.settings.JINJA2_TRACEBACKS:
            return True, self.env.get_template(template_name).render(**template_context)
        ...
        try:
            ...
            logger.error("Exception occurred while rendering %s", e, exc_info=True)
