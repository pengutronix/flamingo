...
class Thumbnails:
    ...
    def media_added(self, context, content, media_content):
        ...
        if "thumbnail" in media_content and not parse_bool(media_content["thumbnail"]):
            logger.debug(
                "setup of thumbnail for %s:%s skipped: disabled by option", content["path"], media_content["path"]
            )
        ...
        if not media_content["width"] and not media_content["height"]:
            logger.debug(
                "setup of thumbnail for %s:%s skipped: no dimensions set", content["path"], media_content["path"]
            )
