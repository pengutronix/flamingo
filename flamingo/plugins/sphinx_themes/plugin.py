...
class SphinxThemes:
        ...
        options = [
            ...
            (
                ...
                [
                    ...
                    for i in sorted(self.sphinx_theme.app.registry.html_themes.keys())
                ],
            ),  # NOQA
            *[
                (
                ...
                for i in sorted(self.settings.keys())
                if i not in ignore
            ],
            "HTML Options",
        ]
    ...
    def build(self):
        ...
        self._raw_theme_options = self.sphinx_theme.config.get_raw_theme_options()
    ...
    def settings_setup(self, context):
        ...
        self.build_dir = os.path.join(self.temp_dir.name, "sphinx-themes/theme")
    ...
    def template_context_setup(self, context, content, template_name, template_context):
        ...
        settings = {key[len("SPHINX_THEMES_") :].lower(): value for key, value in self.settings.items()}
        ...
        sphinx_template_context["css_files"].append(
            StaticFile(os.path.join("/static", sphinx_template_context["stylesheet"]))
        )
