...
class LinkRole:
    ...
    def __call__(self, name, rawtext, text, lineno, inliner):
        ...
        if self.is_media_target(target):
            ...
            return [raw("", '<a href="{}">{}</a>'.format(media_content["url"], name), format="html")], []

        # handle internal links
        abs_lineno = self.rst_base_plugin.offsets.get(self.context.content["path"], 0) + lineno

        if not self.context.settings.PRE_RENDER_CONTENT or not self.context.content.get("is_template", True):
            self.context.logger.error(
                ...
                abs_lineno,
                ("settings" if not self.context.settings.PRE_RENDER_CONTENT else "content file"),
            )
